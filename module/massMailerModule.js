...
import { sendAnnouncementMail, sendEventMail } from "../utilities/mailer/mailer.js";
...
const massMailerModule = {
    ...
    eventParticipantsMailer: async function (eventID, ccEmails, announcement) {
        ...
        try {
            ...
            if (eventData.length === 0) {
                return setResponseBadRequest("Event Doesn't Exist for given Event ID !!");
            }
            ...
            if (userIDs.length > 0) {
                ...
                const userEmails = userData.map(user => user.userEmail);
                await sendEventMail(userEmails, ccEmails, eventData[0].eventName, announcement);
            }   
    ...
    selectedParticipantsMailer: async function (userEmails, ccEmails, announcement) {
        try {
            await sendAnnouncementMail(userEmails, ccEmails, announcement);
            return setResponseOk("Announcement Mail Sent Succussfully !");
        } catch (err) {
