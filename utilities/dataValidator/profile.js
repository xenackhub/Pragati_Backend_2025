...
const validateUserID = (userID) => {
  ...
  if (typeof userID !== "number" || userID <= 0 || !Number.isInteger(userID)) {
    return "Invalid user ID. It must be a positive integer.";
  }
...
const validateProfileData = (userData) => {
  ...
  if (userData.userDepartment && !validateBasicString(userData.userDepartment)) {
    errors.push("Invalid user department.");
  }

  if (userData.academicYear && (typeof userData.academicYear !== "number" || userData.academicYear <= 0)) {
  ...
  if (
    ...
    errors.push("Invalid value for needAccommodationDay1. It must be 0 or 1.");
  }
  ...
  if (
    ...
    errors.push("Invalid value for needAccommodationDay2. It must be 0 or 1.");
  }
  ...
  if (
    ...
    errors.push("Invalid value for needAccommodationDay3. It must be 0 or 1.");
  }

  if (
    userData.isAmrita !== undefined &&
    (typeof userData.isAmrita !== "number" || ![0, 1].includes(userData.isAmrita))
  ) {
  ...
  if (
    ...
    (typeof userData.accountStatus !== "number" || ![0, 1, 2].includes(userData.accountStatus))
  ) {
...
export { validateUserID, validateProfileData };
